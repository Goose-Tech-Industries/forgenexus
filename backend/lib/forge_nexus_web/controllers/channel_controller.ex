defmodule ForgeNexusWeb.ChannelController do
    ...
    categories =
      ...
      |> Enum.map(fn {cat, chs} ->
        %{
          ...
          is_collapsed: cat && cat.is_collapsed,
    ...
    case Channels.get_channel_by_slug(slug) do
      ...
      channel ->
        if Channels.can_view_channel?(user, channel) do
          ...
          conn
          |> json(%{
            channel: channel_detail_json(channel),
    ...
    case Channels.get_channel_by_slug(slug) do
      ...
      channel ->
        if Channels.can_view_channel?(user, channel) do
          ...
          opts =
            if params["limit"],
              do: [{:limit, safe_to_integer(params["limit"], 50)} | opts],
              else: opts
        ...
        if Channels.can_post_in_channel?(user, channel) do
          case Channels.check_slowmode(channel, user) do
            {:error, remaining} ->
              ...
              |> json(%{error: "Slowmode active", seconds_remaining: remaining})
  ...
  def add_reaction(conn, %{"message_id" => message_id} = params) do
    ...
    case Channels.add_reaction(message_id, user.id, params["emoji"]) do
      ...
      {:error, _} ->
        conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to add reaction"})
    end
  end
  ...
  def remove_reaction(conn, %{"message_id" => message_id, "emoji" => emoji}) do
    ...
    case Channels.remove_reaction(message_id, user.id, emoji) do
      ...
      {:error, _} ->
        conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to remove reaction"})
    end
  end
      ...
      case Channels.get_message(message_id) do
        ...
        message ->
          case Channels.pin_message(message, user.id) do
            ...
            {:error, _} ->
              conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to pin"})
          end
      end
    else
      ...
      case Channels.get_message(message_id) do
        ...
        message ->
          case Channels.unpin_message(message) do
            ...
            {:error, _} ->
              conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to unpin"})
          end
      end
    else
  ...
  def update_settings(conn, %{"slug" => slug} = params) do
    ...
    case Channels.get_channel_by_slug(slug) do
      ...
      channel ->
        if params["notification_level"],
          do: Channels.set_notification_level(channel.id, user.id, params["notification_level"])
  ...
  defp channel_json(ch, unread_counts) do
    %{
      ...
      unread_count: Map.get(unread_counts, ch.id, 0)
    }
  end
  ...
  defp reply_preview_json(reply) do
    ...
    %{
      id: reply.id,
      body: String.slice(reply.body || "", 0, 200),
