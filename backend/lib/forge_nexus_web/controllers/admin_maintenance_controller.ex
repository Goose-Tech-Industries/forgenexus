defmodule ForgeNexusWeb.AdminMaintenanceController do
  ...
  def db_stats(conn, _params) do
    ...
    rows =
      Enum.map(tables.rows, fn [name, size, cols] ->
        row_count =
          try do
            ForgeNexus.Repo.query!("SELECT count(*) FROM \"#{name}\"").rows
            |> List.first()
            |> List.first()
          rescue
            _ -> 0
  ...
  def clear_cache(conn, _params) do
    ...
    ForgeNexus.Admin.log_admin_action(admin.id, %{
      ...
      category: "settings",
      description: "Cleared in-memory cache"
    })
    ...
    e ->
      ...
      |> json(%{error: "Cache clear failed: #{Exception.message(e)}"})
  end
  ...
  def reindex_search(conn, _params) do
    ...
    ForgeNexus.Admin.log_admin_action(admin.id, %{
      ...
      category: "settings",
      description: "Triggered search reindex"
    })
