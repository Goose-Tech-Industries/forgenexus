defmodule ForgeNexusWeb.AdminThemeController do
  ...
  def create(conn, %{"theme" => params}) do
    case Themes.create_theme(atomize_keys(params)) do
      {:ok, theme} ->
        ...
        Admin.log_admin_action(admin.id, %{action: "theme_created", category: "themes", target_type: "theme", target_id: theme.id, description: "Created theme #{theme.name}", new_state: params})
  ...
  def update(conn, %{"id" => id, "theme" => params}) do
    ...
    case Themes.update_theme(theme, atomize_keys(params)) do
      {:ok, updated} ->
        admin = Guardian.Plug.current_resource(conn)
        Admin.log_admin_action(admin.id, %{action: "theme_changed", category: "themes", target_type: "theme", target_id: id, description: "Updated theme #{theme.name}"})
  ...
  def delete(conn, %{"id" => id}) do
    ...
    if theme.is_default do
      conn |> put_status(:unprocessable_entity) |> json(%{error: "Cannot delete the default theme"})
    else
      case Themes.delete_theme(theme) do
        {:ok, _} ->
          admin = Guardian.Plug.current_resource(conn)
          Admin.log_admin_action(admin.id, %{action: "theme_deleted", category: "themes", target_type: "theme", target_id: id, description: "Deleted theme #{theme.name}"})
          conn |> json(%{ok: true})
        {:error, _} ->
  ...
  def set_default(conn, %{"id" => id}) do
    import Ecto.Query
    ForgeNexus.Repo.update_all(from(t in "themes", where: t.is_default == true), set: [is_default: false])
    theme = Themes.get_theme!(id)
    case ForgeNexus.Repo.update(Ecto.Changeset.change(theme, is_default: true)) do
      {:ok, updated} ->
        admin = Guardian.Plug.current_resource(conn)
        Admin.log_admin_action(admin.id, %{action: "theme_changed", category: "themes", target_type: "theme", target_id: id, description: "Set #{theme.name} as default theme"})
        conn |> json(%{theme: theme_json(updated)})
      {:error, _} ->
  ...
  defp theme_json(t) do
    %{id: t.id, name: t.name, slug: t.slug, is_active: t.is_active, is_default: t.is_default, position: t.position, colors: Map.drop(Map.from_struct(t), [:__meta__, :id, :name, :slug, :is_active, :is_default, :position, :inserted_at, :updated_at]), inserted_at: t.inserted_at}
  end
