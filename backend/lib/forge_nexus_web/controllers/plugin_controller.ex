defmodule ForgeNexusWeb.PluginController do
  ...
  def create_flow(conn, %{"flow" => flow_params}) do
    ...
    attrs = %{
      name: Map.get(flow_params, "name"),
      description: Map.get(flow_params, "description"),
      slug: slug,
      trigger_type: Map.get(flow_params, "trigger_type"),
      trigger_config: Map.get(flow_params, "trigger_config", %{}),
      settings: Map.get(flow_params, "settings", %{}),
    ...
    case Plugins.create_flow(attrs) do
      {:ok, flow} ->
        ...
        if edges_attrs != [] do
          Plugins.bulk_create_edges(
            flow.id,
            Enum.map(edges_attrs, fn e ->
              %{
  ...
  def update_flow(conn, %{"id" => id, "flow" => flow_params}) do
    attrs =
      flow_params
      |> Map.take(["name", "description", "trigger_type", "trigger_config", "settings"])
  ...
  def delete_flow(conn, %{"id" => id}) do
    case Plugins.delete_flow(id) do
      ...
      {:error, _} ->
        conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to delete flow"})
    end
  end
  ...
  def list_executions(conn, params) do
    opts = [
      flow_id: Map.get(params, "flow_id"),
      status: Map.get(params, "status"),
      limit: parse_int(params, "limit", 25),
      offset: parse_int(params, "offset", 0)
    ]
        ...
        if autosave? do
          ...
          case Plugins.create_flow_from_spec(spec, admin && admin.id) do
            ...
            {:error, reason} ->
              ...
              |> json(%{
                error: "Generated spec rejected by persistence: #{inspect(reason)}",
                spec: spec
              })
          end
        else
  ...
  defp execution_json(exec) do
    %{
      ...
      result: exec.result,
      node_trace: exec.node_trace,
      started_at: exec.started_at,
      finished_at: exec.finished_at,
      duration_ms: exec.duration_ms,
      nodes_executed: exec.nodes_executed,
      db_operations: exec.db_operations,
      http_requests: exec.http_requests,
