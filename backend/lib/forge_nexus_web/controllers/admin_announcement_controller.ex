defmodule ForgeNexusWeb.AdminAnnouncementController do
    ...
    case Repo.insert(Announcement.changeset(%Announcement{}, attrs)) do
      {:ok, ann} ->
        Admin.log_admin_action(user.id, %{
          ...
          target_id: ann.id,
          description: "Created announcement: #{ann.title}"
        })

        conn
        |> put_status(:created)
        |> json(%{announcement: ann_json(Repo.preload(ann, :created_by))})
  ...
  defp ann_json(a) do
    %{
      ...
      created_by:
        if(Ecto.assoc_loaded?(a.created_by) && a.created_by,
          do: %{id: a.created_by.id, username: a.created_by.username}
        )
    }
  end
