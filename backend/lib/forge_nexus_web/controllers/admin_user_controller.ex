defmodule ForgeNexusWeb.AdminUserController do
  ...
  def reset_password(conn, %{"id" => id}) do
    ...
    case Accounts.update_profile(user, %{password: temp_pass}) do
      {:ok, _} ->
        conn |> json(%{temporary_password: temp_pass})

      {:error, _} ->
        conn |> put_status(:unprocessable_entity) |> json(%{error: "Failed to reset password"})
    end
  end

  def bulk_action(conn, %{"action" => action, "user_ids" => user_ids} = params)
      ...
      case Keyword.get(opts, :search) do
        ...
        search ->
          ...
          where(
            query,
            [u],
            ilike(u.username, ^term) or ilike(u.email, ^term) or ilike(u.display_name, ^term)
          )
      end
  ...
  defp admin_user_json(user) do
    %{
      ...
      primary_group:
        if(Ecto.assoc_loaded?(user.primary_group) && user.primary_group,
          do: %{
            id: user.primary_group.id,
            name: user.primary_group.name,
            color: user.primary_group.color,
            is_staff: user.primary_group.is_staff
          }
        )
    }
  end
