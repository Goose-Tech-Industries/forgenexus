defmodule ForgeNexusWeb.AdminForumController do
  ...
  def list_all(conn, _params) do
    ...
    children_query =
      from(c in Forums.Forum, where: is_nil(c.parent_id) == false, order_by: [asc: :position])
  ...
  def create_category(conn, %{"category" => params}) do
    case Forums.create_category(atomize_keys(params)) do
      {:ok, cat} ->
        ...
        conn
        |> put_status(:created)
        |> json(%{category: category_json(ForgeNexus.Repo.preload(cat, :forums))})
    ...
    case Forums.create_forum(atomize_keys(params) |> Map.put(:slug, slug)) do
      {:ok, forum} ->
        ...
        Admin.log_admin_action(admin.id, %{
          ...
          target_type: "forum",
          target_id: forum.id,
          description: "Created forum #{forum.name}",
          new_state: params
        })
    ...
    case ForgeNexus.Repo.update(Forums.Forum.changeset(forum, atomize_keys(params))) do
      {:ok, updated} ->
        ...
        Admin.log_admin_action(admin.id, %{
          ...
          target_type: "forum",
          target_id: id,
          description: "Updated forum #{forum.name}",
          previous_state: previous,
          new_state: params
        })
    ...
    case ForgeNexus.Repo.delete(forum) do
      {:ok, _} ->
        ...
        Admin.log_admin_action(admin.id, %{
          ...
          target_type: "forum",
          target_id: id,
          description: "Deleted forum #{forum.name}"
        })
