defmodule ForgeNexus.Events do
  ...
  def list_events(opts \\ []) do
    ...
    query = if month && year do
      start_date = Date.new!(year, month, 1) |> DateTime.new!(~T[00:00:00])
      end_date = Date.new!(year, month, Date.days_in_month(Date.new!(year, month, 1))) |> DateTime.new!(~T[23:59:59])
  ...
  def list_upcoming(limit \\ 5) do
    Event
    |> where([e], e.is_published == true and e.is_cancelled == false and e.starts_at >= ^DateTime.utc_now())
    |> order_by(asc: :starts_at)
    |> limit(^limit)
    |> preload(:created_by)
    |> Repo.all()
  ...
  def rsvp(event_id, user_id, status \\ "going") do
    case Repo.get_by(EventRsvp, event_id: event_id, user_id: user_id) do
      nil ->
        %EventRsvp{} |> EventRsvp.changeset(%{event_id: event_id, user_id: user_id, status: status}) |> Repo.insert()
      existing ->
  ...
  def suggest_events do
    ...
    gaming_forums = Enum.filter(forums, fn f ->
      name = String.downcase(f.name || "")
      String.contains?(name, ["gaming", "game", "esport", "play"])
    end)
    suggestions = if length(gaming_forums) > 0 do
      suggestions ++ [%{
    ...
    creative_forums = Enum.filter(forums, fn f ->
      name = String.downcase(f.name || "")
      String.contains?(name, ["art", "creative", "design", "media", "photo", "music"])
    end)
    suggestions = if length(creative_forums) > 0 do
      suggestions ++ [%{
    ...
    help_forums = Enum.filter(forums, fn f ->
      name = String.downcase(f.name || "")
      String.contains?(name, ["help", "support", "question", "faq", "tutorial"])
    end)
    suggestions = if length(help_forums) > 0 do
      suggestions ++ [%{
    ...
    suggestions ++ [
      %{title: "Monthly Town Hall", description: "Community updates, feedback, and open discussion with staff.", event_type: "meetup", suggested_day: "1st of month", reason: "Town halls build community trust and reduce surprise policy changes"},
      %{title: "New Member Welcome Mixer", description: "Meet and greet for members who joined this month!", event_type: "community", suggested_day: "Last Friday of month", reason: "Welcoming new members improves 30-day retention by 40%"},
      %{title: "Forum Trivia Night", description: "Test your knowledge of the community! Prizes for winners.", event_type: "contest", suggested_day: "Thursday", reason: "Trivia events drive engagement spikes and create memorable moments"}
    ]
  end
end
