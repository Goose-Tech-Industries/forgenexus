defmodule ForgeNexus.Predictions do
  ...
  alias ForgeNexus.Predictions.{Prediction, PredictionOption, PredictionBet, Suggestion, SuggestionVote}
  ...
  def list_predictions(status \\ nil) do
    ...
    if status, do: where(query, [p], p.status == ^status), else: query
    |> Repo.all()
  end
    ...
    if prediction.status != "open" do
      ...
      Repo.transaction(fn ->
        case ForgeNexus.Economy.deduct_points(user_id, amount, "prediction_bet") do
          {:ok, _} ->
            bet = %PredictionBet{}
            |> PredictionBet.changeset(%{user_id: user_id, prediction_id: prediction.id, option_id: option_id, amount: amount})
            |> Repo.insert!()
            from(o in PredictionOption, where: o.id == ^option_id) |> Repo.update_all(inc: [total_amount: amount])
            bet
  ...
  def resolve_prediction(prediction_id, winning_option_label) do
    ...
    Repo.transaction(fn ->
      if winning_pool > 0 do
        winning_bets = Repo.all(from b in PredictionBet, where: b.prediction_id == ^prediction_id and b.option_id == ^winning_option.id)
      ...
      prediction |> Prediction.changeset(%{status: "resolved", winning_option: winning_option_label, resolved_at: now}) |> Repo.update!()
    end)
  end
  ...
  def vote_suggestion(suggestion_id, user_id, direction) when direction in [:up, :down] do
    ...
    case Repo.one(from v in SuggestionVote, where: v.suggestion_id == ^suggestion_id and v.user_id == ^user_id) do
     nil ->
        Repo.transaction(fn ->
          %SuggestionVote{} |> SuggestionVote.changeset(%{suggestion_id: suggestion_id, user_id: user_id, direction: dir_str}) |> Repo.insert!()
      ...
      existing ->
        if existing.direction == dir_str do
          ...
          Repo.transaction(fn ->
            ...
            {inc_field, dec_field} = if direction == :up, do: {[upvotes: 1], [downvotes: -1]}, else: {[downvotes: 1], [upvotes: -1]}
            from(s in Suggestion, where: s.id == ^suggestion_id) |> Repo.update_all(inc: inc_field ++ dec_field)
            :ok
          end)
        end
    end
