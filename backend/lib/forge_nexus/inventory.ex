defmodule ForgeNexus.Inventory do
  ...
  def create_item_template(attrs), do: %ItemTemplate{} |> ItemTemplate.changeset(attrs) |> Repo.insert()
  ...
  def give_item(user_id, item_template_id, quantity \\ 1) do
    ...
    if template.is_stackable do
      case Repo.one(from ii in InventoryItem, where: ii.user_id == ^user_id and ii.item_template_id == ^item_template_id) do
       nil ->
          %InventoryItem{} |> InventoryItem.changeset(%{user_id: user_id, item_template_id: item_template_id, quantity: quantity}) |> Repo.insert()
        existing ->
          new_qty = min(existing.quantity + quantity, template.max_stack)
          existing |> InventoryItem.changeset(%{quantity: new_qty}) |> Repo.update()
      end
    else
      results = Enum.map(1..quantity, fn _ ->
        %InventoryItem{} |> InventoryItem.changeset(%{user_id: user_id, item_template_id: item_template_id, quantity: 1}) |> Repo.insert()
      end)
  ...
  def remove_item(user_id, item_template_id, quantity \\ 1) do
    case Repo.one(from ii in InventoryItem, where: ii.user_id == ^user_id and ii.item_template_id == ^item_template_id, limit: 1) do
     nil -> {:error, :item_not_found}
      item ->
        new_qty = item.quantity - quantity
        if new_qty <= 0, do: Repo.delete(item), else: item |> InventoryItem.changeset(%{quantity: new_qty}) |> Repo.update()
    end
  end

  def has_item?(user_id, item_template_id) do
    Repo.exists?(from ii in InventoryItem, where: ii.user_id == ^user_id and ii.item_template_id == ^item_template_id and ii.quantity > 0)
  end

  def get_inventory(user_id) do
    InventoryItem |> where([ii], ii.user_id == ^user_id and ii.quantity > 0) |> preload(:item_template) |> order_by(:inserted_at) |> Repo.all()
  end
  ...
  def equip_item(user_id, inventory_item_id) do
    item = Repo.one!(from ii in InventoryItem, where: ii.id == ^inventory_item_id and ii.user_id == ^user_id, preload: :item_template)
    Repo.transaction(fn ->
      from(ii in InventoryItem,
        join: t in ItemTemplate, on: ii.item_template_id == t.id,
        where: ii.user_id == ^user_id and ii.is_equipped == true and t.category == ^item.item_template.category and ii.id != ^inventory_item_id
