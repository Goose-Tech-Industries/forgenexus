defmodule ForgeNexus.Applications do
  ...
  def list_all_forms, do: ApplicationForm |> order_by(asc: :title) |> preload(:target_group) |> Repo.all()
  def get_form!(id), do: Repo.get!(ApplicationForm, id) |> Repo.preload(:target_group)

  def create_form(attrs), do: %ApplicationForm{} |> ApplicationForm.changeset(attrs) |> Repo.insert()
  def update_form(id, attrs), do: get_form!(id) |> ApplicationForm.changeset(attrs) |> Repo.update()
  ...
  def review_application(app_id, reviewer_id, status, note \\ nil) do
    ...
    result = app
      |> StaffApplication.changeset(%{status: status, reviewer_id: reviewer_id, review_note: note, reviewed_at: now})
      |> Repo.update()
  ...
  defp has_pending_application?(form_id, user_id) do
    StaffApplication
    |> where([a], a.form_id == ^form_id and a.user_id == ^user_id and a.status in ["pending", "under_review"])
    |> Repo.exists?()
  end
end
