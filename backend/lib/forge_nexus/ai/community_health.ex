defmodule ForgeNexus.AI.CommunityHealth do
  ...
  def calculate(community_id) do
    ...
    overall =
      (engagement * 0.35 + moderation * 0.25 + growth * 0.20 + retention * 0.20)
      |> Float.round(1)
      |> min(100.0)
      |> max(0.0)
  ...
  defp engagement_score(community_id) do
    ...
    posts =
      from(p in "posts",
        where: p.community_id == type(^community_id, :binary_id) and p.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
    ...
    threads =
      from(t in "threads",
        where: t.community_id == type(^community_id, :binary_id) and t.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()

    voice_sessions =
      from(c in "voice_call_logs",
        where: c.community_id == type(^community_id, :binary_id) and c.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
  ...
  defp moderation_score(community_id) do
    ...
    reports =
      from(r in "reports",
        where: r.community_id == type(^community_id, :binary_id) and r.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
    ...
    bans =
      from(b in "bans",
        where: b.community_id == type(^community_id, :binary_id) and b.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
  ...
  defp growth_score(community_id) do
    ...
    this_week =
      from(u in "users",
        where: u.community_id == type(^community_id, :binary_id) and u.inserted_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
    ...
    last_week =
      from(u in "users",
        where:
          u.community_id == type(^community_id, :binary_id) and u.inserted_at > ^fourteen_days and
            u.inserted_at <= ^seven_days,
        select: count()
      )
      |> Repo.one()
  ...
  defp retention_score(community_id) do
    ...
    total_members =
      from(u in "users",
        where: u.community_id == type(^community_id, :binary_id),
        select: count()
      )
      |> Repo.one()
    ...
    active_members =
      from(u in "users",
        where: u.community_id == type(^community_id, :binary_id) and u.last_seen_at > ^seven_days,
        select: count()
      )
      |> Repo.one()
