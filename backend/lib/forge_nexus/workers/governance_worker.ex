defmodule ForgeNexus.Workers.GovernanceWorker do
  ...
  def perform(_job) do
    ...
    Enum.each(expired_votes, fn proposal ->
      ...
      passed =
        meets_participation &&
          case proposal.threshold_type do
            ...
            "two_thirds" ->
              proposal.yes_count >= (total_votes - proposal.abstain_count) * 2 / 3

            "three_quarters" ->
              proposal.yes_count >= (total_votes - proposal.abstain_count) * 3 / 4
      ...
      summary =
        "Votes: #{proposal.yes_count} yes, #{proposal.no_count} no, #{proposal.abstain_count} abstain. #{if meets_participation, do: "Quorum met.", else: "Quorum not met."}"
