defmodule ForgeNexus.Workers.AnalyticsDailyWorker do
  ...
  def perform(_job) do
    ...
    new_members =
      from(u in User, where: u.inserted_at >= ^start_dt and u.inserted_at <= ^end_dt)
    ...
    active_creators =
      from(t in Thread,
        where: t.inserted_at >= ^start_dt and t.inserted_at <= ^end_dt,
        select: t.user_id,
        distinct: true
      )
      |> Repo.all()
    ...
    posts_created =
      from(p in Post, where: p.inserted_at >= ^start_dt and p.inserted_at <= ^end_dt)
      |> Repo.aggregate(:count)

    threads_created =
      from(t in Thread, where: t.inserted_at >= ^start_dt and t.inserted_at <= ^end_dt)
      |> Repo.aggregate(:count)

    # Response rate: % of threads created yesterday that have at least 1 reply
    threads_with_replies =
      if threads_created > 0 do
        from(t in Thread,
