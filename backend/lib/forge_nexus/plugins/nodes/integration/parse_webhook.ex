defmodule ForgeNexus.Plugins.Nodes.Integration.ParseWebhook do
  ...
  def validate_config(config) do
    case Map.get(config, "mappings") do
      ...
      _ ->
        {:error, ["mappings must be a JSON object of {output_field: json_path}"]}
    end
  end
  ...
  def schema do
    %{
      ...
      label: "Parse Webhook",
      description:
        "Extracts fields from a webhook payload using dot-separated JSON path mappings.",
      inputs: [
      ...
      config_fields: [
        %{
          ...
          default: %{},
          description:
            "JSON object mapping output field names to dot-separated paths in the payload"
        }
      ]
    }
  end
