defmodule ForgeNexus.Tournaments do
  ...
  def get_tournament!(id), do: Repo.get!(Tournament, id) |> Repo.preload([:participants, :matches])

  def register_participant(tournament_id, user_id) do
    tournament = Repo.get!(Tournament, tournament_id)
    participant_count = Repo.one(from p in Participant, where: p.tournament_id == ^tournament_id, select: count(p.id))
    cond do
      tournament.status != "registration" -> {:error, :registration_closed}
      tournament.max_participants && participant_count >= tournament.max_participants -> {:error, :tournament_full}
      true ->
        %Participant{}
        |> Participant.changeset(%{tournament_id: tournament_id, user_id: user_id, seed: participant_count + 1})
        |> Repo.insert()
    end
  end
  ...
  defp generate_bracket(tournament_id, participants, round) do
    ...
    Enum.with_index(pairs, 1) |> Enum.map(fn {pair, pos} ->
      attrs = %{tournament_id: tournament_id, round: round, position: pos, status: "active"}
      attrs = case pair do
        [p1, p2] -> Map.merge(attrs, %{player1_id: p1.user_id, player2_id: p2.user_id})
        [p1] -> Map.merge(attrs, %{player1_id: p1.user_id, winner_id: p1.user_id, status: "bye"})
      end
  ...
  def get_current_matchup(tournament_id, user_id) do
    Repo.one(
      from m in Match,
        where: m.tournament_id == ^tournament_id and m.status == "active" and (m.player1_id == ^user_id or m.player2_id == ^user_id),
        limit: 1
    )
  end
  ...
  def submit_result(match_id, winner_id, scores) do
    ...
    Repo.transaction(fn ->
      match |> Match.changeset(%{winner_id: winner_id, scores: scores, status: "completed"}) |> Repo.update!()
      loser_id = if match.player1_id == winner_id, do: match.player2_id, else: match.player1_id
      from(p in Participant, where: p.tournament_id == ^match.tournament_id and p.user_id == ^winner_id)
      |> Repo.update_all(inc: [wins: 1, points: 3])
      from(p in Participant, where: p.tournament_id == ^match.tournament_id and p.user_id == ^loser_id)
  ...
  def advance_bracket(tournament_id) do
    current_round = Repo.one(from m in Match, where: m.tournament_id == ^tournament_id, select: max(m.round))
    pending = Repo.one(from m in Match, where: m.tournament_id == ^tournament_id and m.round == ^current_round and m.status == "active", select: count(m.id))
    if pending > 0 do
      {:error, :matches_still_active}
    else
      winners = Repo.all(from m in Match, where: m.tournament_id == ^tournament_id and m.round == ^current_round and not is_nil(m.winner_id), select: m.winner_id)
  ...
  def get_standings(tournament_id) do
    ...
    |> select([p, u], %{user_id: u.id, username: u.username, wins: p.wins, losses: p.losses, points: p.points, is_eliminated: p.is_eliminated})
    |> Repo.all()
  end
