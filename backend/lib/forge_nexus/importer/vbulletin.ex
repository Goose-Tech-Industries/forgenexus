defmodule ForgeNexus.Importer.VBulletin do
  @moduledoc """
  Import adapter for vBulletin 3.x/4.x forum exports.

  Expects JSON data in the universal import format with source: "vbulletin".

  vBulletin-specific handling:
  - Timestamps: stored as Unix timestamps (dateline fields)
  - Categories: forums with parentid = -1 are categories
  - BBCode: vBulletin uses standard BBCode with some custom tags
  - Custom BBCode tags: [QUOTE=username;postid], [HIGHLIGHT], [INDENT], [THREAD], [POST]
  - Passwords: uses MD5 + salt, cannot be migrated — require reset
  """
    ...
    id_map =
      ...
      |> Enum.reduce(%{}, fn {user_data, index}, acc ->
        ...
        attrs = %{
          ...
          email: user_data["email"] || "imported_vb_#{user_data["source_id"]}@placeholder.local",
    ...
    post_id_map =
      ...
      |> Enum.reduce(%{}, fn {post_data, index}, acc ->
        ...
        unless thread_id && user_id do
          ...
          body = normalize_vbulletin_bbcode(post_data["body"] || "")
  ...
  # --- vBulletin-specific helpers ---

  @doc """
  Normalize vBulletin-specific BBCode to standard BBCode.
  Handles custom tags like [QUOTE=user;postid], [HIGHLIGHT], [INDENT], etc.
  """
  def normalize_vbulletin_bbcode(text) when is_binary(text) do
    text
    # Convert vBulletin QUOTE with username and post ID to standard quote
    |> String.replace(~r/\[QUOTE=([^;\]]+);[^\]]*\]/i, "[QUOTE=\\1]")
    # Convert [HIGHLIGHT] to [B] (bold as fallback)
    |> String.replace(~r/\[HIGHLIGHT\]/i, "[B]")
    |> String.replace(~r/\[\/HIGHLIGHT\]/i, "[/B]")
    # Convert [INDENT] to plain text (no standard BBCode equivalent)
    |> String.replace(~r/\[INDENT\]/i, "")
    |> String.replace(~r/\[\/INDENT\]/i, "")
    # Convert [THREAD=id] and [POST=id] to plain text
    |> String.replace(~r/\[THREAD=\d+\]([^\[]*)\[\/THREAD\]/i, "\\1")
    |> String.replace(~r/\[POST=\d+\]([^\[]*)\[\/POST\]/i, "\\1")
    # Normalize [ATTACH] tags (remove, as attachments aren't migrated)
    |> String.replace(~r/\[ATTACH[^\]]*\]\d+\[\/ATTACH\]/i, "[Attachment]")
  end

  def normalize_vbulletin_bbcode(nil), do: ""
